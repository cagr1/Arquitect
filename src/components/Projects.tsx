    ...
    <section id="work" className="py-32 md:py-64 bg-[#0a0a0a]">
      <div className="px-6 md:px-12 mb-24 md:mb-40 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        <div className="reveal">
          <span className="text-[10px] uppercase tracking-[0.3em] opacity-40 block mb-6">03 / Works</span>
          <h2 className="font-serif text-6xl md:text-9xl leading-[0.8]">Selected <br /><span className="italic">Output.</span></h2>
        </div>
        <div className="max-w-xs reveal" style={{ transitionDelay: '0.2s' }}>
          <p className="text-sm md:text-base font-light opacity-50 leading-relaxed mb-6">
            A curated archive of residential and cultural interventions that redefine their surroundings.
          </p>
          <a href="#" className="text-[10px] uppercase tracking-widest border-b border-white/20 pb-1 hover:border-white transition-all">Download Index (PDF)</a>
        </div>
      </div>
              ...
              <div className={`${isFullWidth ? 'w-full' : 'md:w-3/5'} group cursor-pointer relative`}>
                ...
                <div className="mt-8 flex flex-col md:flex-row md:items-end md:justify-between space-y-4 md:space-y-0">
                  <div className="flex items-start space-x-6">
                    ...
                    <div>
                      <h3 className="font-serif text-3xl md:text-5xl group-hover:italic transition-all duration-500">{project.title}</h3>
                      <div className="flex items-center space-x-4 mt-2">
                        <p className="text-[10px] uppercase tracking-widest opacity-40">{project.location}</p>
                        <span className="w-1 h-1 bg-white/20 rounded-full"></span>
                        <p className="text-[10px] uppercase tracking-widest opacity-40">{project.category}</p>
                      </div>
                    </div>
                  </div>
      ...
      <div className="mt-48 md:mt-80 flex flex-col items-center">
        <button className="group relative">
           <span className="font-serif text-4xl md:text-7xl transition-all duration-700 group-hover:italic group-hover:tracking-wider">Explore full archive</span>
